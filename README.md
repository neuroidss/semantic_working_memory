...
$$\mathbf{\Psi}_k(p) = \sum_{t} \left( P_{\gamma_k, i}(t) \cdot P_{\gamma_k, j}^*(t) \right) w_k(t)$$
...
$$\mathbf{E}_{\text{drift}} = \operatorname{Interp}_{32 \to 77}(\mathbf{h}_{0..31}) \in \mathbb{R}^{1 \times 77 \times 768}$$
$$\mathbf{E}_{\text{conditioning}} = \text{Encode}(\text{""}) + \alpha \cdot \frac{\mathbf{E}_{\text{drift}}}{\|\mathbf{E}_{\text{drift}}\|}$$
