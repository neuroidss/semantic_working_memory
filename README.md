...
To prevent chaotic out-of-distribution latent trajectories, the 120-pair phase vectors are projected onto the top 120 singular vectors $\mathbf{V}_{120} \in \mathbb{R}^{120 \times 768}$ computed via SVD from the complete 49,408-token [CLIP](https://openai.com) vocabulary embedding matrix $\mathbf{E}_{\text{vocab}}$:
